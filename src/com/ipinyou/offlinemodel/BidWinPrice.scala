...
object BidWinPrice {
  ...
  def main (args: Array[String]) {
    ...
    val filterdata = data.filter(bidpricefilter)
    val pricedata = filterdata.map(priceProcess)

    val platformprice = pricedata.reduceByKey((a,b) => a+b)

    //platformprice.repartition(1).saveAsTextFile("/Users/ggstar/Desktop/priceplatform_firstprice")
    platformprice.repartition(1).saveAsTextFile("/user/data-usermodel/price_dist/price_filter2")
  ...
  def priceProcess(log:String): (String, Int) ={
    ...
    var priceKey = platform

    (platform, 1)
  }


  def bidpricefilter(log:String): Boolean ={
    var logs = log.split('\u0003')
    var implog = logs.apply(0).split('\u0001')
    var bidlog = logs.apply(1).split('\u0001')
    ...
    if(site.length > 2){
      auctionType = bidlog.apply(7).split('\u0002').apply(2)
    }
    if(site.length > 6){
      width = bidlog.apply(7).split('\u0002').apply(6)
    }
    ...
    auctionType.equals("second_price") /*&& bidPrice.toDouble.toInt == winprice.toDouble.toInt && winprice.toDouble > floorprice*/
