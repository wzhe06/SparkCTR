...
class FMModel(val task: Int,
  ...
  def predict(testData: DataFrame): DataFrame = {
    import testData.sparkSession.implicits._
    testData.rdd.mapPartitions{
      _.map{
        row => {
          val positiveProbability = predict(Vectors.fromML(row.getAs[org.apache.spark.ml.linalg.DenseVector]("scaledFeatures")))
          ( row.getAs[Int]("label"), org.apache.spark.ml.linalg.Vectors.dense(Array(1 - positiveProbability, positiveProbability)))
        }
      }
